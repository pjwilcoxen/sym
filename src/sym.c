...
char *argv[];
{
   ...
   if (isoption("dd", 2))
      debugforce = 1;
   if (isoption("d", 1) && !(isoption("debug", 2) || isoption("dd", 2)))
      debug = 1;
   if (isoption("first", 1))
      only_first = 1;
   if (isoption("last", 1))
      only_last = 1;
   if (isoption("merge_only", 1))
      mergeonly = 1;
   if (isoption("scalars", 2))
      do_scalars = 1;
   ...
   if (mergeonly == 0)
   {
      lang = 0;
      for (thislang = known->first; thislang; thislang = thislang->next)
         if (isoption(thislang->str, strlen(thislang->str)))
            lang = thislang->str;
